...
5. Do **not** delete the original files in `outputs/individual/`. The digest is a view on top of them, not a replacement.
...
- The provenance block on each individual output makes this command cheap to build — every output already carries its prompt path and summary.
